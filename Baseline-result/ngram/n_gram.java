...
public class n_gram {
	...
	public static String all_dataset[] = { "activemq", "camel", "derby", "groovy", "hbase", "hive", "jruby", "lucene",
			"wicket" };
	public static String all_train_releases[] = { "activemq-5.0.0", "camel-1.4.0", "derby-10.2.1.6", "groovy-1_5_7",
			"hbase-0.94.0", "hive-0.9.0", "jruby-1.1", "lucene-2.3.0", "wicket-1.3.0-incubating-beta-1" };
	...
	public static ModelRunner train_model(String train_release) {
		...
		Lexer lexer = new WhitespaceLexer(); // Use a Java lexer; if your code is already lexed, use whitespace or
												// tokenized lexer
		// Lexer lexer = new TokenizedLexer();
	...
	public static void predict_defective_lines(String train_release, String test_release, ModelRunner modelRunner)
			throws Exception {
		...
		for (int j = 0; j < java_files.length; j++) {
			...
			for (int i = 0; i < linenum.size(); i++) {
				...
				for (int k = 0; k < lineTokens.size(); k++) {
					...
					sb.append(train_release + "\t" + test_release + "\t" + filename_original + "\t" + cur_line_num
							+ "\t" + tok + "\t" + tok_score + "\t" + averageEntropy + "\n");
				}
